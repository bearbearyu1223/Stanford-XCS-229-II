...
def softmax(x):
    ...
    shift_x = x - (np.max(x, axis=1)).reshape(x.shape[0], 1)
    p = np.exp(shift_x) / np.sum(np.exp(shift_x), axis=1, keepdims=True)
    return p
    # *** END CODE HERE ***
...
def forward_pass(W, S):
    ...
    return softmax(np.dot(S, W).reshape(-1, W.shape[1]))
    # *** END CODE HERE ***
