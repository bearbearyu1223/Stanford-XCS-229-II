...
from submission import init_policy_data, choose_action, record_transition, accumulate_discounted_future_rewards, forward_pass
...
def record_episode(tracked, session_index, episode_index, num_steps, total_reward, policy_data, env):
  ...
  if accumulate_discounted_future_rewards(np.array([]),0.0) is not None:
    S_hist, A_hist, G_hist = [np.array(x) for x in zip(*policy_data['history'])]
  ...
  tracked['pi'].append(forward_pass(policy_data['W'],
                                    np.array([convert_state_one_hot(s) for s in env.all_states], dtype=np.float64)))
