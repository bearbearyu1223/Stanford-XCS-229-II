...
class Model:
    ...
    def build_model(self):
        ...
        np.random.seed(10)
        self.model.add(
            Bidirectional(LSTM(units=200, activation='relu', input_shape=(self.input_time_steps, self.input_dim),
                               return_sequences=True)))
        self.model.add(Dropout(self.dropout_rate))
        self.model.add(Bidirectional(LSTM(units=100, activation='relu', return_sequences=False)))
        self.model.add(Dropout(self.dropout_rate))
    ...
    def train(self, x: np.array, y: np.array, epochs: int, batch_size: int, save_dir: str):
        """Train the model"""
        print("[Phase::Train]")
        file_name = os.path.join(save_dir, '%s-%s-e%s-h%s-p%s.h5' % ("bi-lstm",
